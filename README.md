...
### 11. Сбор статических файлов (для production)

```bash
python manage.py collectstatic
```

> В `settings.py` нужно предварительно раскомментировать `STATIC_ROOT`.
...
| Категория               | Изменения                                                                        |
| ----------------------- | -------------------------------------------------------------------------------- |
| **Django**              | Обновлён с 4.1.4 до 5.2                                                          |
| **Зависимости**         | Обновлены все пакеты в requirements.txt                                          |
| **PEP 8**               | Исправлены все нарушения стиля кода                                              |
| **Документация**        | Добавлены docstring ко всем функциям и классам                                   |
| **Комментарии**         | Подробные комментарии в Python-коде и шаблонах                                   |
| **Баги**                | Исправлена опечатка `last_url_elemen`, добавлен `return` в POST-обработчике      |
| **settings.py**         | SECRET_KEY вынесен в переменную окружения, включён STATICFILES_DIRS              |
| **Шаблоны**             | Исправлены незакрытые теги, лишние теги вне extends, Bootstrap обновлён до 5.3.3 |
| **URL-маршруты**        | Добавлены имена (`name=`) ко всем маршрутам responseApp                          |
| **store_result**        | Переписан с использованием `getlist()` вместо ручного цикла                      |
| **Тесты**               | Добавлены базовые тесты для всех приложений                                      |
| **objects_arrays.html** | Исправлено отображение внешних и локальных изображений                           |
| **Статика**             | Исправлены пути к статическим файлам                                             |
